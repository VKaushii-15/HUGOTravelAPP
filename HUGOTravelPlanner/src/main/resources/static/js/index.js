...
const getPlaceIds = (e) => {
  ...
  if (!titleElm || !titleElm.nextElementSibling) return null;

  const itemElms = [...titleElm.nextElementSibling.children];
  if (itemElms.length < 2) return null;
...
const createTripObj = () => {
  const days_attributes = [...document.querySelectorAll(".daily")].map(
      (plannerBox) => {
          ...
          const [day, month, year] = dateStr.split('/'); // Split using '/'
          const date = new Date(`${year}-${month}-${day}`); // Create a date object in 'yyyy-mm-dd' format
          ...
          return { date: date.toISOString().split('T')[0], places_attributes };  
      }
  ).filter(day => day !== null); // Filter out any invalid days
